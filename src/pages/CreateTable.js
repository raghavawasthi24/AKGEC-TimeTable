...
const CreateTable = () => {
  const fetchinfo = () => {
    axios
      .get(
        "https://time-table-production.up.railway.app/departmentss/select_teachers/2"
      )
  ...
  const Post = () => {
    axios
      .post(
        "https://time-table-production.up.railway.app/departmentss/create_table/",
        {
          teacher_id: Teacher,
          subject_id: subjectid,
          class_id: [classid],
          no_of_lectures: parseInt(lecture),
          type: type,
        }
      )
  ...
  return (
    <div className="maintable">
      <div style={{ fontSize: "3rem", width: "17rem", marginLeft: "-2rem" }}>
        Create Table
      </div>
      <div className="field">
        <select
        ...
        >
          ...
          {teacherdata.map((teachername, i) => (
            <option value={teachername.id} key={i}>
              {teachername.user}
            </option>
          ))}
